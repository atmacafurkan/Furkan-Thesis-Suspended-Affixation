...
acceptance_ci <- readRDS("./acceptance_ci.rds")

init_responses <- readRDS("./init_judgments.rds")
df <- readRDS("./df_judgments.rds")

init_subject <- init_responses %>% summarise(x= length(unique(subject)))
final_subject <- df %>% summarise(x= length(unique(subject)))

# linear mixed effects model
modelim_df <- readRDS("./sa_acceptability.rds") %>%
  mcmc_intervals_data(pars = vars(starts_with("b_")), prob_outer = 0.95) %>%
  subset(!str_detect(parameter,":")) %>% subset(parameter !="b_Intercept")


modelim_df$parameter %<>% dplyr::recode(`b_conjoiner1` = "veya",
                                        `b_Suffix1` = "-(I)msI",
                                        `b_Suffix2` = "-(I)ncI",
                                        `b_Suffix3` = "-(ş)Ar",
                                        `b_Suffix4` = "-CAsInA",
...
df_corpus <- readRDS("./suffixfrequency.rds")
...
The participants were \Sexpr{length(unique(init_responses$subject))} students from Boğaziçi University who are native speakers of Turkish. In exchange for their participation they received 1 point to their overall course score.
...
<<derivationalmodel, echo=FALSE, out.width='\\linewidth', fig.width=4, fig.height=3, fig.align='center', fig.pos='hbt!', fig.cap='First experiment, model results fit to grammaticality judgments with the predictors Suffix(8 derivational|1 inflectional) and Conjoiner(ve|veya)', dev='cairo_pdf'>>=
modelim_df %>% ggplot(aes(m, y=factor(parameter, 
...
df_corpus %>% ggplot(aes(suffix, ratio, fill = suffix)) +
...
There are many examples for the SA of \textit{-(I)ncI} within $\sim$500 hits. The same can not be said for \textit{-(I)msI} which has the same acceptance rate as \textit{-(I)ncI} but the corpus search does not result in an SA of \textit{-(I)msI} within $\sim$500 hits. This discrepency between similar acceptabilities but different corpus results can be explained by the relative frequency of these suffixes according to the context they are used in. The examples for SA of \textit{-(I)ncI} mostly comprise of texts written by clerks or reporters that refer to the passage or paragrap numbers of a law. In (\ref{corpusquerry}), I give some partial examples from the corpus search results for (\ref{inciquerry}). 
